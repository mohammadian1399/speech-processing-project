...
class Dataset(BaseDataset):
    ...
    def _select_noise_y(self, target_length):
        ...
        while remaining_length > 0:
            ...
            # If you need to add new noise, insert a small mute section
    ...
    def snr_mix(clean_y, noise_y, snr, target_dB_FS, target_dB_FS_floating_value, rir=None, eps=1e-6):
        """
        Mixed noise and pure speech, when the rir parameter is not empty, reverberation effect is applied to pure speech
        ...
        # Use noisy's rms to scale audio
        noisy_y, _, noisy_scalar = tailor_dB_FS(noisy_y, noisy_target_dB_FS)
        clean_y *= noisy_scalar

        # There may be clipping when synthesizing noisy speech, although very rarely
        # Adjust noisy, clean_y, noise_y slightly
